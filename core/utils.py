        ...
        for palm in pair_palm:
            if centroid_trace:
                distance = euclidean_distance(
                    ball[0], ball[1], palm[0], palm[1])
            else:
                distance = euclidean_distance(
                    ball["centroid_point"][0], ball["centroid_point"][1], palm[0], palm[1])
...
def classification(image, bound_ball_pair, prev_pair, pair_ball, ptn_model):
    ...
    if len(bound_ball_pair) > 0:
        for bound_ball in bound_ball_pair:
            ...
            # recognize pattern
            results.append(pattern_recognition(
                bound_ball, image_w, image_h, ptn_model))
...
def draw_bbox(image, bound_ball_pair, pair_ball, pair_palm):
    ...
    for ball in pair_ball:
        ...
        cv2.putText(image, text, (int(ball["p1"][0]), int(
            ball["p1"][1])+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), int(0.6 * (image_h + image_w) / 600))

    # draw bbox on bound ball
    for bound_ball in bound_ball_pair:
        cv2.rectangle(image, bound_ball["p1"],
                      bound_ball["p2"], (0, 255, 0), 3)
        text = "ball bound"
    ...
    # draw bbox on ball
    for palm in pair_palm:
        cv2.rectangle(image, (palm[2][0]-25, palm[2][1]-25),
                      (palm[3][0]+25, palm[3][1]+25), (0, 0, 255), 3)
        cv2.putText(image, "palm", (int(palm[2][0]), int(
            palm[2][1])+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), int(0.6 * (image_h + image_w) / 600))
