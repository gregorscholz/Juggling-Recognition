...
def display_demo_pattern(demo, ptns):
    ...
    # display pattern
    cv2.putText(demo, "Pattern: ", (pos_x, pos_y), font_style,
                font_size, (255, 255, 255), font_scale)
    x_axis = (resolution)//8  # pattern result pos
    for ptn in ptns:
        cv2.putText(demo, str(ptn[0]), (int(
            x_axis), pos_y+int(pos_y*0.06)), font_style, font_size, ptn[1], font_scale)
...
def display_demo_palm(demo, palm):
    ...
    xmax = (palm[0]) + 15
    ymax = (palm[1]) + 15
    cv2.rectangle(demo, (int(xmin), int(ymin)),
