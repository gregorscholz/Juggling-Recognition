...
class poseDetector():
    ...
    def findPosition(self, img, draw=True):
        ...
        if self.results.pose_landmarks:
            for id, lm in enumerate(self.results.pose_landmarks.landmark):
                ...
                if draw:
                    cv2.circle(img.astype(np.uint8),
                               (int(cx), int(cy)), 5, (255, 0, 0), cv2.FILLED)
        return self.lmList
    ...
    def find_Elbow_angle(self, image, demo):
        ...
        cv2.circle(
            demo, (right_elbow[1], right_elbow[2]), 15, (0, 0, 255), cv2.FILLED)
        cv2.circle(demo, (left_elbow[1], left_elbow[2]),
    ...
    def findPalm(self):
        ...
        palm_sequence = [self.lmList[16][1:], self.lmList[18]
                         [1:], self.lmList[20][1:], self.lmList[22][1:]]
        ...
        palm_sequence = [self.lmList[15][1:], self.lmList[17]
                         [1:], self.lmList[19][1:], self.lmList[21][1:]]
        min_x = min(palm_sequence, key=lambda x: x[0])[0]
        min_y = min(palm_sequence, key=lambda x: x[1])[1]
        max_x = max(palm_sequence, key=lambda x: x[0])[0]
        max_y = max(palm_sequence, key=lambda x: x[1])[1]
    ...
    def distance_estimation(self, frame):
        ...
        distance = int(
            math.sqrt((left_pinky[0]-left_index[0])**2 + (left_pinky[1]-left_index[1])**2))
