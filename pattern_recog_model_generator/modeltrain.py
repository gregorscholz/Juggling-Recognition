...
parser.add_argument('--loss', default='categorical_crossentropy',
                    type=str, help='loss function to be used')
parser.add_argument('--lr', default=1, type=float, help='learning rate')
parser.add_argument('--epochs', default=20, type=int, help='epochs quantity')
parser.add_argument('--bs', default=16, type=int, help='batch size')
parser.add_argument('--checkpoint', default='pattern_model.h5',
                    type=str, help='path for model saving')
...
parser.add_argument('--x_data', default=os.path.realpath(os.path.dirname(__file__)) + '/data/x.npy',
                    type=str, help='x data file name')
parser.add_argument('--y_data', default=os.path.realpath(os.path.dirname(__file__)) + '/data/y.npy',
                    type=str, help='y data file name')
...
class Model:
    ...
    def train_model(self):
        # compile model
        opt = optimizers.adam_v2.Adam(learning_rate=self.learning_rate)
        checkpoints = ModelCheckpoint(
            self.model_filename, monitor='val_accuracy', verbose=1, save_best_only=True, period=1)
