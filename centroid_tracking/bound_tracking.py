    ...
    for bp in ball_pair:
        for bbp in bound_ball_pair:
            ...
            distance_list.append(
                [distance, bp["ID"], bbp["ID"], bbp["hand_seq"], bbp["p1"], bbp["p2"], bbp["centroid_point"]])

    for i in range(len(ball_pair)):
        if len(distance_list) < 1:
            continue
        shortest = min(distance_list)
        [distance_list.remove(item) for item in distance_list if item[2]
         == shortest[2] or item[1] == shortest[1]]
        for ball in ball_pair:
            if ball["ID"] == shortest[1]:
                ...
                break
        [bound_ball_pair.remove(
            bbp) for bbp in bound_ball_pair[:] if bbp["ID"] == shortest[2]]
...
def p2_track(location, ball_pair, bound_ball_pair, frame):
    ...
    for ball in predict_balls:
        ...
        cv2.circle(frame, np.array(ball["centroid_point"]).astype(
            int), 15, ball["colors"], thickness=cv2.FILLED)
